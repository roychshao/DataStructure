...
#include <fstream>
#include <iomanip>
#define MAX_LEVEL 100    // change original max level 6 to 100 , for not to restrict the level
const float P = 0.5;
...
int main() 
{
    ...
    bool search = false;
    srand(time(NULL));
    ofstream result;    /* store experiment result in the file "skip_result_o.csv" */
    result.open("skip_result_o.csv");
    while(k < 31)
    {
        skiplist ss;
        int i = 0, length = 1 << k;
...
int random_level() 
{
    ...
    //  the original random approach is hard to understand
    //  I use positive probility 0.5 for easier to understand
